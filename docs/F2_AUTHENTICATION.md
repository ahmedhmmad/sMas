...
**الحالة:** D1/M24 🔒 (CI `3f4ae05`) · D2/M25 🔒 (CI `62c76b1`) · D3/M26 🔒 (CI `bdfce09`) · D4 ⚠️ دلالات A/B/C (§5)
...
**قرارات المراجعة (2026-09-26):**

| # | |
|---|---|
| R1 | ✅ **معتمدة كـF2 defaults:** رمز 5 د؛ 5 محاولات؛ طلب جديد يلغي السابق؛ 5 إخفاقات ⇒ 15 د؛ OTP يفك القفل — تبقى configuration متى سمح التصميم (نقل الثوابت من M26 متابعة)؛ **الموظف `active` فقط** (`on_leave` لا جلسة) |
| R2 | ✅ تنبيهات القفل لا تُضاف الآن — المرحلة 5 (قناة الإرسال) |
| R3 | ✅ **Tenant Admin Auth identity remains native email for now; separate decision deferred.** |
| R4 | ✅ `login_challenges` بلا T7 معتمد — لا OTP hashes في `audit_log`؛ النتائج تُدقَّق على الكيان |
| R5 | حد المحاولات في ذاكرة العملية (كـD1)؛ ✅ **I4 = Production deployment requirement** |
...
✅ **الحروف معتمدة بترتيب PLAN (2026-09-26)** — حفاظاً على الترتيب الحرفي بدل ربط مستقل. غير معرَّف أيضاً في PLAN: ما يحدث في الدخول التالي بعد أول دخول (النمط 2: هل يبقى OTP فقط؟)، والنمط الافتراضي لمدرسة لم تضبطه.
