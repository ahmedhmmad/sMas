...
select pg_temp.rec('meta.execute', $q$select count(*)::text from fns f, unnest(array['anon','authenticated','public']) g
...
select is((select v from r where k = 'meta.execute'), '0',    'no EXECUTE for anon, authenticated or PUBLIC until M20');
